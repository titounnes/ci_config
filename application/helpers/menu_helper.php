    ...
    function menuTree($data, $route='',$deep=0)
    {
        if(is_array($data))
        {
            ...
            {    
                ...
                {
                    echo '<li>';
                    echo '<a style="padding-top:9px;padding-bottom:9px;" href="#" data-href="'.$route.'/view/'.$key.'/'.($val).'" class="btn-load btn-ajax-page" data-target="#workspace" level="'.$deep.'"><i class="fa fa-'.(lang($key.'_icon')=='' || !is_array($val) ?'caret-right':lang($key.'_icon')).'"></i>'.(lang($route.ucfirst($key))==''?$key:lang($route.ucfirst($key))).'</a>';
                }
                if(is_array($val))
                {
